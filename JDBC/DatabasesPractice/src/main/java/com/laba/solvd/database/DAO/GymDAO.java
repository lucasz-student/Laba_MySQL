...
public class GymDAO implements IDAO<Gym>, ICreatableNoRelationship<Gym>{
	...
	public List<Gym> selectAll() {
		...
		try (Connection connection = (Connection) ConnectionManager.getSQLConnection().get();) {
			PreparedStatement ps = connection.prepareStatement(String.format(GET_ALL, "gym"));
	...
	public Gym selectById(int Id) {
		Gym gym = new Gym();
		try (Connection connection = (Connection) ConnectionManager.getSQLConnection().get();) {
			PreparedStatement psGymFields = connection.prepareStatement(String.format(GET_BY_ID, "gym"));
